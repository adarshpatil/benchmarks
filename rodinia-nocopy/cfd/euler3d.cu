...
__global__ void cuda_initialize_variables(int nelr, float* variables, float* ff_variable)
{
...
void initialize_variables(int nelr, float* variables, float* ff_variable)
{
	...
	cuda_initialize_variables<<<Dg, Db>>>(nelr, variables, ff_variable);
 ...
 *
*/
__global__ void cuda_compute_flux(int nelr, int* elements_surrounding_elements, float* normals, float* variables, float* fluxes, float* ff_variable, float3* ff_flux_contribution_momentum_x, float3* ff_flux_contribution_momentum_y, float3* ff_flux_contribution_momentum_z, float3* ff_flux_contribution_density_energy)
{
...
{
	dim3 Dg(nelr / block_length), Db(block_length);
	cuda_compute_flux<<<Dg,Db>>>(nelr, elements_surrounding_elements, normals, variables, fluxes, ff_variable, ff_flux_contribution_momentum_x, ff_flux_contribution_momentum_y, ff_flux_contribution_momentum_z, ff_flux_contribution_density_energy);
...
int main(int argc, char** argv)
{
	...
	//{
		...
		std::cout << "Elements: " << nel << " nelr: " << nelr << " NNB: " << NNB << " NDIM: " << NDIM << std::endl;
		// read in data
		for(int i = 0; i < nel; i++)
		{
			file >> h_areas[i];
		...
		std::cout << "Filled in the rest..." << std::endl;
	...
	float* variables = (float*)malloc(sizeof(float)*nelr*NVAR);
	...
	float* old_variables = (float*)malloc(sizeof(float)*nelr*NVAR);
	float* fluxes = (float*)malloc(sizeof(float)*nelr*NVAR);
	float* step_factors = (float*)malloc(sizeof(float)*nelr);	
    ...
    memcpy(ff_variable, h_ff_variable, NVAR*sizeof(float)); // We may be able to remove this copy...
   ...
   // CUDA_SAFE_CALL( cudaMemcpyToSymbol(ff_flux_contribution_momentum_y, &h_ff_flux_contribution_momentum_y, sizeof(float3)) );
	...
	for(int i = 0; i < iterations; i++)
	{
		...
		memcpy(old_variables, variables, nelr*NVAR);

		// for the first iteration we compute the time step
		compute_step_factor(nelr, variables, h_areas, step_factors);
		CUT_CHECK_ERROR("compute_step_factor failed");

		for(int j = 0; j < RK; j++)
		{
			compute_flux(nelr, h_elements_surrounding_elements, h_normals, variables, fluxes, ff_variable, &h_ff_flux_contribution_momentum_x, &h_ff_flux_contribution_momentum_y, &h_ff_flux_contribution_momentum_z, &h_ff_flux_contribution_density_energy);
			CUT_CHECK_ERROR("compute_flux failed");
