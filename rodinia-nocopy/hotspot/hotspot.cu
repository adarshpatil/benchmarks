...
void run(int argc, char** argv)
{
    ...
    float *FilesavingTemp[2],*FilesavingPower,*MatrixOut;
    ...
    FilesavingTemp[0] = (float *) malloc(size*sizeof(float));
    FilesavingTemp[1] = (float *) malloc(size*sizeof(float));
    FilesavingPower = (float *) malloc(size*sizeof(float));
    MatrixOut = (float *) calloc (size, sizeof(float));

    if( !FilesavingPower || !FilesavingTemp[0] || !FilesavingTemp[1] || !MatrixOut)
        fatal("unable to allocate memory");

    printf("pyramidHeight: %d\ngridSize: [%d, %d]\nborder:[%d, %d]\nblockGrid:[%d, %d]\ntargetBlock:[%d, %d]\n",\
	pyramid_height, grid_cols, grid_rows, borderCols, borderRows, blockCols, blockRows, smallBlockCol, smallBlockRow);

    readinput(FilesavingTemp[0], grid_rows, grid_cols, tfile);
...
//     cudaMemcpy(MatrixTemp[0], FilesavingTemp, sizeof(float)*size, cudaMemcpyHostToDevice);
    ...
    writeoutput(FilesavingTemp[ret],grid_rows, grid_cols, ofile);
